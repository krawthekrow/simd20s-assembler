	...
	Utils.mapStatements(block, {
		...
		screen: (statement) => {
			...
			if(statement.updateGraphicsBuffer){
				statement.dataReg = expandToken(statement.dataReg);
				statement.dataReg = normaliseGpuReg(statement.dataReg);
				valid = valid && validateType(statement.dataReg, ['gpuRegWithModifiers']);
			}
